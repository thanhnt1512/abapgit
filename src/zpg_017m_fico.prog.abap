...
METHOD zii_si_zfii017m_bp_in~si_zfii017m_bp_in.
  ...
  CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
    EXPORTING
      input  = ls_cvis_ei_extern-partner-header-object_instance-bpartner
    IMPORTING
      output = ls_cvis_ei_extern-partner-header-object_instance-bpartner.
  ...
  ls_cvis_ei_extern-partner-central_data-common-data-bp_person-firstname = 'X'.
  ls_cvis_ei_extern-partner-central_data-common-data-bp_person-correspondlanguage = 'E'.
  ...
  LOOP AT input-mt_zfii017m_bp_in-bp_maintain-bankdetails INTO DATA(ls_bankdetail).
    CHECK ls_bankdetail-task IS NOT INITIAL.
    IF ls_bankdetail-task <> 'I' AND input-mt_zfii017m_bp_in-bp_maintain-object_task = 'I'.
      CONTINUE.
    ENDIF.
    ...
    UNASSIGN <fs_bankdetails>.
  ENDLOOP.

  "CUSTOMER

  IF input-mt_zfii017m_bp_in-bp_maintain-customer_data-task IS NOT INITIAL AND input-mt_zfii017m_bp_in-bp_maintain-customer_data-task = input-mt_zfii017m_bp_in-bp_maintain-object_task .
    ls_cvis_ei_extern-customer-header-object_task = input-mt_zfii017m_bp_in-bp_maintain-customer_data-task.
    IF input-mt_zfii017m_bp_in-bp_maintain-customer_data-task = input-mt_zfii017m_bp_in-bp_maintain-object_task.
      APPEND INITIAL LINE TO ls_cvis_ei_extern-partner-central_data-role-roles ASSIGNING FIELD-SYMBOL(<fs_role>).
      <fs_role>-task = input-mt_zfii017m_bp_in-bp_maintain-customer_data-task.
      IF <fs_role>-task = 'U'.
      ...
      IF <fs_company_cus>-task = 'U'.
        ...
        ls_cvis_ei_extern-customer-central_data-central-datax-konzs = 'X'.
      ENDIF.
      ...
      ELSEIF input-mt_zfii017m_bp_in-bp_maintain-customer_data-ecommerce_pfm_flag = ''.
        ls_cvis_ei_extern-customer-central_data-central-data-konzs = ''.
      ENDIF.

      "BP_COMMERCE
      IF input-mt_zfii017m_bp_in-bp_maintain-customer_data-ecommerce_bp IS NOT INITIAL.
        IF input-mt_zfii017m_bp_in-bp_maintain-customer_data-task = 'U'.
*        SELECT SINGLE partner FROM but000 INTO @DATA(lw_partner) WHERE  partner_guid = @ls_cvis_ei_extern-partner-header-object_instance-bpartnerguid.
          SELECT SINGLE partner1 FROM but050 INTO @DATA(lw_partner1) WHERE partner2 = @ls_cvis_ei_extern-partner-header-object_instance-bpartner.
          IF lw_partner1 IS NOT INITIAL.
            APPEND INITIAL LINE TO ls_cvis_ei_extern-partner_relation ASSIGNING FIELD-SYMBOL(<fs_partner_relation>).
            <fs_partner_relation>-header-object_task = 'D'.
            <fs_partner_relation>-central_data-main-task = 'D'.
            <fs_partner_relation>-header-object_instance-partner1-bpartner = lw_partner1.
            <fs_partner_relation>-header-object_instance-partner2-bpartner = ls_cvis_ei_extern-partner-header-object_instance-bpartner.
            ...
            CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
              EXPORTING
                input  = <fs_partner_relation>-header-object_instance-partner1-bpartner
              IMPORTING
                output = <fs_partner_relation>-header-object_instance-partner1-bpartner.
            <fs_partner_relation>-header-object_instance-relat_category = 'ZBP3'.
            UNASSIGN <fs_partner_relation>.
          ENDIF.
        ELSEIF input-mt_zfii017m_bp_in-bp_maintain-customer_data-task = 'I'.
          APPEND INITIAL LINE TO ls_cvis_ei_extern-partner_relation ASSIGNING <fs_partner_relation>.
          <fs_partner_relation>-header-object_task = 'I'.
          <fs_partner_relation>-central_data-main-task = 'I'.
          <fs_partner_relation>-header-object_instance-partner1-bpartner = input-mt_zfii017m_bp_in-bp_maintain-customer_data-ecommerce_bp.
      ...
      IF input-mt_zfii017m_bp_in-bp_maintain-object_task = 'I' AND input-mt_zfii017m_bp_in-bp_maintain-customer_data-task = 'I'.
        LOOP AT input-mt_zfii017m_bp_in-bp_maintain-customer_data-extend_sales_org INTO DATA(ls_extend_sales_org).
          ...
          <fs_sale>-task = input-mt_zfii017m_bp_in-bp_maintain-customer_data-task.
          <fs_sale>-data_key-vkorg = ls_extend_sales_org-sales_org.
          <fs_sale>-data-waers = 'VND'.
          <fs_sale>-data_key-vtweg = '01'.
          <fs_sale>-data_key-spart = '01'.

          APPEND INITIAL LINE TO <fs_sale>-functions-functions ASSIGNING FIELD-SYMBOL(<fs_functions>).
          <fs_functions>-task = 'I'.
          <fs_functions>-data_key-parvw = 'AG'.
          <fs_functions>-data_key-parza = 000.

          APPEND INITIAL LINE TO <fs_sale>-functions-functions ASSIGNING <fs_functions>.
          <fs_functions>-task = 'I'.
          <fs_functions>-data_key-parvw = 'RE'.
          <fs_functions>-data_key-parza = 000.

          APPEND INITIAL LINE TO <fs_sale>-functions-functions ASSIGNING <fs_functions>.
          <fs_functions>-task = 'I'.
          <fs_functions>-data_key-parvw = 'RG'.
          <fs_functions>-data_key-parza = 000.

          APPEND INITIAL LINE TO <fs_sale>-functions-functions ASSIGNING <fs_functions>.
          <fs_functions>-task = 'I'.
          <fs_functions>-data_key-parvw = 'WE'.
          <fs_functions>-data_key-parza = 000.

          UNASSIGN <fs_sale>.
          UNASSIGN <fs_functions>.
        ENDLOOP.
      ENDIF.
    ENDIF.
  ENDIF.
  ...
  ELSEIF input-mt_zfii017m_bp_in-bp_maintain-customer_data-all_sorg_flag = ''.
    ls_cvis_ei_extern-customer-central_data-central-data-katr1 = ''.
  ENDIF.
  ...
  LOOP AT input-mt_zfii017m_bp_in-bp_maintain-customer_data-po_reconciliation INTO DATA(ls_po_reconciliation).
    ...
    IF  <fs_partner_relation>-central_data-main-task = 'M'.
...
*    <fs_partner_relation>-central_data-main-task = 'I'.
  ...
  UNASSIGN <fs_role>.
  IF input-mt_zfii017m_bp_in-bp_maintain-vendor_data-task IS NOT INITIAL AND input-mt_zfii017m_bp_in-bp_maintain-customer_data-task = input-mt_zfii017m_bp_in-bp_maintain-object_task .
    ls_cvis_ei_extern-vendor-header-object_task = input-mt_zfii017m_bp_in-bp_maintain-vendor_data-task.
    IF ls_cvis_ei_extern-vendor-header-object_task = input-mt_zfii017m_bp_in-bp_maintain-object_task.
      APPEND INITIAL LINE TO ls_cvis_ei_extern-partner-central_data-role-roles ASSIGNING <fs_role>.
      <fs_role>-task = input-mt_zfii017m_bp_in-bp_maintain-vendor_data-task.
      IF <fs_role>-task = 'U'.
        <fs_role>-datax-valid_to = 'X'.
      ENDIF.
      ...
      <fs_company_ven>-data-akont = '1411200000'.
